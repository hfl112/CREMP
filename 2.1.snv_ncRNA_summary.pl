...
while(<SNV>){
    ...
    $mutation{$chr}{$pos}{$type1}="$alt~$type2~$num~$sample";
}


my %count;
open POS, "</mnt/data/hfn/Data/GRCh38.80/Chrpos_RNApos/Chrpos_RNApos_ok.txt" or die $!;
while(<POS>){
    chomp;
    my @t = split /\t/;
    my $chr = $t[0];
    ...
    if(defined $snv{$t[0]}{$t[1]}){
        for(my $i=0;$i< @{$snv{$t[0]}{$t[1]}}; $i++){
            ...
            my ($alt,$type2,$num,$sample) = split /~/,$mutation{$chr}{$pos}{$mut};


            print "$chr\t$pos\t$ref_original\t$alt\t$gene_id\t$gene_type\t$id\t$transcript_type\t$gene_symbol\t$strand";

            if($strand eq "-"){$alt =~ tr/ATCG/TAGC/;} #CHANGE TO RNA sequence
            print "\t$chr\t$rna_pos\t$rna_ref_original\t$alt\t$rna_ref_original$rna_pos$alt\t${$snv{$t[0]}{$t[1]}}[$i]\t$type2\t$num\t$sample\t$anno\n";
		}
	}
}
